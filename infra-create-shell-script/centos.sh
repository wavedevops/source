...
SUBNET_ID="$SUBNET_US_EAST_1F"  # Specify your subnet ID here
...
AMI_ID=$(aws ec2 describe-images --region ${REGION} --filters "Name=name,Values=Centos-8-DevOps-Practice" | jq -r '.Images[].ImageId')
if [ -z "${AMI_ID}" ]; then
